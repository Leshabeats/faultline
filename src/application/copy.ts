...
export const applicationCopy = {
  en: {
    ...
    workspaceNodeLimitDetail: (limit: number) => `A workspace can contain at most ${limit} components.`,
    workspaceEdgeLimitDetail: (limit: number) => `A workspace can contain at most ${limit} connections.`,
  ...
  ru: {
    ...
    workspaceLimitReached: 'Достигнут лимит доски',
    workspaceNodeLimitDetail: (limit: number) => `На доске может быть не более ${limit} компонентов.`,
    workspaceEdgeLimitDetail: (limit: number) => `На доске может быть не более ${limit} связей.`,
